...
//                  not use the syntax:   href=" scwShow(...)"
...
var scwBaseYear = scwDateNow.getFullYear() - 111;

// How many years do want to be valid and to show in the drop-down list?
var scwDropDownYears = 130;
...
function scwShow(scwEle, scwSource) {
    ...
    if (scwDateValue.length == 0) {
        ...
        function scwInputFormat() {
            ...
            var scwExpValDay = new RegExp('^(0?[1-9]|[1-2][0-9]|3[0-1])$'), scwExpValMonth = new RegExp('^(0?[1-9]|1[0-2]|' + scwArrMonthNames.join('|') + ')$', 'i'), scwExpValYear = new RegExp('^([0-9]{1,2}|[0-9]{4})$');
    ...
    if (scwAutoPosition) {
        var scwWidth = parseInt(scwID('scw').offsetWidth, 10), scwHeight = parseInt(scwID('scw').offsetHeight, 10), scwWindowLeft = (document.body && document.body.scrollLeft) ? document.body.scrollLeft//DOM compliant
...
function scwShowMonth(scwBias) {
    ...
    if (scwTemp >  - 1 && scwTemp < (12 * scwDropDownYears)) {
        ...
        function scwSetOutput(scwOutputDate) {
            ...
            scwHide();
            //alert('ok 2====='+scwTargetEle.value);
            var lDate = null;
            var FDate = null;

            if (scwTargetEle.id == 'idArrivalDate' || scwTargetEle.id == 'iadate') {
                lDate = new Date();
                lDate.setDate(lDate.getDate() - 1);
                FDate = parseDate(scwTargetEle.value, 'mm/dd/yyyy');

                var ok = validateDate(scwTargetEle, lDate, FDate, 'Intended arrival should be a future date');

                if (ok) {
                    var dateDiff = daysBetween(FDate, lDate);
                    ok = checkMaxNo('ARRD', scwTargetEle, dateDiff, 1);
                }

                if (ok && scwTargetEle.id == 'iadate') {
                    var indenAryDa = document.getElementById("pedate").value;
                    if (ok && indenAryDa != '') {
                        lDate = parseDate(indenAryDa, 'mm/dd/yyyy');
                        var dateDiff = daysBetween(lDate, FDate);
                        //                        alert(dateDiff);
                        ok = checkMaxNo('INAD', scwTargetEle, dateDiff, 2);
                    }
                }

            }

            if (scwTargetEle.id == 'idDobDate' || scwTargetEle.id == 'idDDobDate' || scwTargetEle.id == 'bdate' || scwTargetEle.id == 'dDobDate' || scwTargetEle.id == 'idReEnteredDDobDate' ||scwTargetEle.id == 'idReEnteredDobDate' || scwTargetEle.id == 'dob') {
                lDate = new Date();
                FDate = parseDate(scwTargetEle.value, 'mm/dd/yyyy');
                var ok = validateDate(scwTargetEle, FDate, lDate, 'Date of Birth can not be a future date');

                if (ok && scwTargetEle.id == 'idDobDate') {
                    var psIs = document.getElementById("idPassIsueDate").value;
                    if (psIs != '') {
                        lDate = parseDate(psIs, 'mm/dd/yyyy');
                        //                        alert(lDate);
                        ok = validateDate(scwTargetEle, FDate, lDate, 'Date of Birth should be earlier to passport issued date');
                    }
                }
                if (ok && (scwTargetEle.id == 'idDDobDate' || scwTargetEle.id == 'dDobDate')) {
                    var dateDiff = daysBetween(lDate, FDate);
                    ok = checkMaxNo('DAGE', scwTargetEle, dateDiff, 1);
                }

                if (ok && (scwTargetEle.id == 'idDDobDate' || scwTargetEle.id == 'dDobDate')) {

                    var dateDiff = daysBetween(lDate, FDate);
                    ok = checkMaxNo('DCAG', scwTargetEle, dateDiff, 1);
                }
                if (ok && scwTargetEle.id == 'idDDobDate') {
                    var psIs = document.getElementById("idDobDate").value;
                    FDate = parseDate(psIs, 'mm/dd/yyyy');
                    lDate = parseDate(scwTargetEle.value, 'mm/dd/yyyy');
                    ok = validateDate(scwTargetEle, FDate, lDate, 'Child Date of Birth cannot be prior to applicant date of birth');
                }
                if (ok && scwTargetEle.id == 'dDobDate') {
                    var psIs = document.getElementById("bdate").value;
                    FDate = parseDate(psIs, 'mm/dd/yyyy');
                    lDate = parseDate(scwTargetEle.value, 'mm/dd/yyyy');
                    ok = validateDate(scwTargetEle, FDate, lDate, 'Child Date of Birth cannot be prior to applicant date of birth');
                }
                ...
                if (ok && scwTargetEle.id == 'idDobDate') {
                    var psIs = document.getElementById("idDDobDate").value;
                    if (psIs != null && psIs != '') {
                        FDate = parseDate(psIs, 'mm/dd/yyyy');
                        lDate = parseDate(scwTargetEle.value, 'mm/dd/yyyy');
                        ok = validateDate(scwTargetEle, lDate, FDate, 'Applicant Date of Birth cannot be later to Child date of birth ');
                    }
                }
                if (ok && scwTargetEle.id == 'bdate') {
                    var psIs = document.getElementById("dDobDate").value;
                    if (psIs != null && psIs != '') {
                        FDate = parseDate(psIs, 'mm/dd/yyyy');
                        lDate = parseDate(scwTargetEle.value, 'mm/dd/yyyy');
                        ok = validateDate(scwTargetEle, lDate, FDate, 'Applicant Date of Birth cannot be later to Child date of birth ');
                ...
                if (ok && scwTargetEle.id == 'bdate') {
                    var psIs = document.getElementById("pidate").value;
                    if (psIs != '') {
                        lDate = parseDate(psIs, 'mm/dd/yyyy');
                        //                        alert(lDate);
                        ok = validateDate(scwTargetEle, FDate, lDate, 'Date of Birth should be earlier to passport issued date');
            ...
            if (scwTargetEle.id == 'idPassIsueDate') {
                lDate = new Date();
                FDate = parseDate(scwTargetEle.value, 'mm/dd/yyyy');
                var ok = validateDate(scwTargetEle, FDate, lDate, 'Passport issued date cannot be future date');

                if (ok) {
                    var dobk = document.getElementById("idDobDate").value
                    lDate = parseDate(dobk, 'mm/dd/yyyy');
                    //                    alert(lDate);
                    ok = validateDate(scwTargetEle, lDate, FDate, 'Passport issued date should be later to birth date');
                }

                if (ok) {
                    var arrDatek = document.getElementById("idHiddenArrivalDate").value
                    lDate = parseDate(arrDatek, 'mm/dd/yyyy');
                    ok = validateDate(scwTargetEle, FDate, lDate, 'Passport issued date should be prior to intended arrival date');
                }

            }
            if (scwTargetEle.id == 'idPassExpDate') {
                lDate = new Date();
                FDate = parseDate(scwTargetEle.value, 'mm/dd/yyyy');
                var ok = validateDate(scwTargetEle, FDate.lDate, 'Passport expiry date should be a future date');
                if (ok) {
                    var arrDatek = document.getElementById("idHiddenArrivalDate").value
                    lDate = parseDate(arrDatek, 'mm/dd/yyyy');
                    ok = validateDate(scwTargetEle, lDate, FDate, 'Passport expiry date should be later to arrival date');
                }
                if (ok) {
                    var dateDiff = daysBetween(FDate, lDate);
                    //                    alert(dateDiff);
                    ok = checkMaxNo('PEXD', scwTargetEle, dateDiff, 2);
                }
            }

            if (scwTargetEle.id == 'pidate') {
                lDate = new Date();
                FDate = parseDate(scwTargetEle.value, 'mm/dd/yyyy');
                var ok = validateDate(scwTargetEle, FDate, lDate, 'Passport issued date cannot be future date');
                var bdatex = document.getElementById("bdate").value;
                if (ok && bdatex != '') {
                    lDate = parseDate(bdatex, 'mm/dd/yyyy');
                    ok = validateDate(scwTargetEle, lDate, FDate, 'Passport issued date should be later to birth date');
                }
                //
                var indenAryDa = document.getElementById("iadate").value;
                if (ok && indenAryDa != '') {
                    lDate = parseDate(indenAryDa, 'mm/dd/yyyy');
                    ok = validateDate(scwTargetEle, FDate, lDate, 'Passport issued date should be prior to intended arrival date');
                }
            }
            if (scwTargetEle.id == 'pedate') {
                //alert('idate');
                lDate = new Date();
                FDate = parseDate(scwTargetEle.value, 'mm/dd/yyyy');
                //                alert('lDate'+lDate);
                //                alert('fDate'+FDate);
                var ok = validateDate(scwTargetEle, lDate, FDate, 'Passport expiry date should be a future date');
                var indenAryDa = document.getElementById("iadate").value;
                if (ok && indenAryDa != '') {
                    var dateDiff = daysBetween(FDate, lDate);
                    lDate = parseDate(indenAryDa, 'mm/dd/yyyy');
